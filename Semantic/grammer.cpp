...
// ---------------------- 用于语法分析器中的跟踪调试
void grammer_class::enter(char * x)		{ cout << "enter in  "    << x << endl; }
void grammer_class::back(char * x)		{ cout << "exit from  "   << x << endl; }
void grammer_class::call_match(char * x)		{ cout << "matchtoken	" << x << endl; } 
void grammer_class::tree_trace(tree_node_ptr x)	{ print_syntax_tree(x, 1); }
